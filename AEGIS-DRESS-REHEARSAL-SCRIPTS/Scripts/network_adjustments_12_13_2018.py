...
def main():
    ...
    process('wdi_tests.txt', 'update')


def process(file_name, what):
...
def extract(table_name):
    ...
    sql_string = 'SELECT rowid, g3e_geometry from ' + schema + '.' + table_name + ' rec WHERE mdsys.sdo_relate (rec.G3E_GEOMETRY,' + geom_type + elem_info + ordinate_array + options

    # SQL statement for full extract
    #sql_string = 'select rowid, geom from ' + schema + '.' + table_name
...
def update(table_name):
    ...
    data_file = os.path.join(main_dir, 'updates/' + table_name + '.txt')
