...
namespace GTechnology.Oncor.CustomAPI
{
	public class fiPhasePosition : IGTFunctional
	{
		...
		public const string set3 = "N,E,W,S";
		...
		private List<string> m_PhasePositionList = null;
		private List<string> m_PhaseList = null;

		private List<string> m_phasePositions = null;
		...
		private bool CheckPhasePositionSets()
		{
			...
			if (phasePositionRs != null && phasePositionRs.RecordCount > 0)
			{
				...
				//{
				//	phasePosition = phasePosition + ","+ Convert.ToString(phasePositionRs.Fields["PHASE_POS_C"].Value);
		...
		private bool CheckPhaseAndPhasePosition(out bool isPhaseExists)
		{
			...
			if (m_PhasePositionList == null)
			{
				GetPickListPhasePositions();
			}
		...
		private void GetPickListPhasePositions()
		{
			Recordset phasePositionRs = GetRecordSet("select vl_value from VL_PHASE_POSITION");
			if (phasePositionRs != null && phasePositionRs.RecordCount > 0)
			{
				m_PhasePositionList = new List<string>();
				phasePositionRs.MoveFirst();
				while (!phasePositionRs.EOF)
				{
					m_PhasePositionList.Add(Convert.ToString(phasePositionRs.Fields["vl_value"].Value));
					phasePositionRs.MoveNext();
				}
			}
		}
