...
namespace GTechnology.Oncor.CustomAPI
{
    ...
    internal static class dataProvider //we may want to use singleton here, just to be fancy
    {
        ...
        static string emailBody = "GIS ESI Location Request attached";
        ...
        private static bool SubmitEmail(string fileName)
        {
            bool returnValue = false;
            try
            {
                string[] tmpArr = { UserPrincipal.Current.EmailAddress };
                List<string> toAddresses = new List<string>(tmpArr);
                Recordset paramGrabber = new Recordset();
                string toAddress = "SELECT * FROM SYS_GENERALPARAMETER WHERE PARAM_NAME = ? AND SUBSYSTEM_NAME  = ?";
                string efURL = "SELECT * FROM SYS_GENERALPARAMETER WHERE PARAM_NAME = ? AND SUBSYSTEM_NAME  = ?";
                paramGrabber = gtDataContext.OpenRecordset(toAddress, CursorTypeEnum.adOpenDynamic, LockTypeEnum.adLockOptimistic, -1,"TO_ADDRESS", "REQUEST_ESI_LOCATION_CC");
                if (!paramGrabber.EOF && !paramGrabber.BOF)
                {
                    paramGrabber.MoveFirst();
                    while (!paramGrabber.EOF)
                    {
                        toAddresses[0] += "; " + paramGrabber.Fields["PARAM_VALUE"].Value as string;
                        paramGrabber.MoveNext();
                    }
                    SendEmail.SendEmail sender = new SendEmail.SendEmail();
                    string[] addresses = toAddresses.ToArray();
                    sender.EmailRequest.ToAddrs = addresses;
                    sender.GTDataContext = gtDataContext;
                    sender.EmailRequest.Attachments = fileName;
                    sender.EmailRequest.FromAddr = UserPrincipal.Current.EmailAddress;
                    sender.EmailRequest.Subject = "GIS ESI Location Request";
                    sender.EmailRequest.Message = emailBody;
                    paramGrabber = gtDataContext.OpenRecordset(efURL, CursorTypeEnum.adOpenDynamic, LockTypeEnum.adLockOptimistic, -1, "EF_URL", "SEND_EMAIL");
                    paramGrabber.MoveFirst();
                    sender.EFUrl = paramGrabber.Fields["PARAM_VALUE"].Value as string;
                    returnValue = sender.sendEmail();
            ...
            {
                MessageBox.Show("Unable to get current user's email address. Need to be on the network.", "G/Technology");
                returnValue = false;
            }
            return returnValue;
        }
