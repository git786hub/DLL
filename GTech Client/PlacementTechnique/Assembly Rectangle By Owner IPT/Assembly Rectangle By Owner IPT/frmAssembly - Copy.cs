    ...
    public partial class frmAssembly : Form
    {
        ...
        private void frmAssembly_Load(object sender, EventArgs e)
        {
			try
			{
				int cnt = 0;
				ADODB.Recordset rs;
				string assemblyTable = DetailIdentifier == 0 ? "E$PRISWGEAR_P" : "E$PRISWGEAR_DP";
				
				string sql = "SELECT G3E_IDENTIFIER,CT.G3E_FNO,G3E_DESCRIPTION FROM CATALOGENTRYLIST ct, :1 PS WHERE G3E_FNO=:2 AND CT.G3E_IDENTIFIER=PS.G3E_FID";

				rs = m_GTApplication.DataContext.Execute(sql, out cnt, (int)ADODB.CommandTypeEnum.adCmdText,assemblyTable, FeatureFNO);
				if (rs != null)
				{
					if (!(rs.EOF && rs.BOF))
					{
